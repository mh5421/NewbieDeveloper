...
public class CustomerView extends JPanel 
{
	...
	public void registCustomer(){
		...
		ct.setCustName(tfCustName.getText());
		ct.setCustTel1(tfCustTel.getText());
		ct.setCustTel2(tfCustTelAid.getText());
		ct.setCustAddr(tfCustAddr.getText());
		ct.setCustEmail(tfCustEmail.getText());
	...
	public void searchByTel(){
		...
		try {
			...
			Customer  customer = model.selectByTel(tel);
			// 3. 2번의 넘겨받은 Customer의 각각의 값을 화면 텍스트 필드 지정
			tfCustTel.setText(customer.getCustTel1());
			tfCustName.setText(customer.getCustName());
			tfCustTelAid.setText(customer.getCustTel2());
	...
	public void updateCustomer(){
		...
		int result;
		try {
			result = model.updateCustomer(ct);
			if(result ==1) {
